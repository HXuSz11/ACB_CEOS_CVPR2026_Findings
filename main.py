...
if __name__ == "__main__":
    ...
    print(
        "Dataset: {}, N task: {}, Large First Task Classes: {}, Classes Per Task : {}".format(
            args.dataset,
            args.n_task,
            args.n_class_first_task,
            class_per_task,
    ...
    cl_train_val = ContinualLearningDataset(
        train_set,
        task_dict,
        args.n_task,
        args.n_class_first_task,
    ...
    cl_test = ContinualLearningDataset(
        test_set,
        task_dict,
        args.n_task,
        args.n_class_first_task,
    ...
    test_loaders = [
        DataLoader(test, batch_size=args.batch_size * 4, shuffle=False, num_workers=args.nw)
    ...
    if args.valid_size > 0:
        ...
        train_loaders = [
            DataLoader(train, batch_size=args.batch_size, shuffle=True, num_workers=args.nw)
            for train in train_dataset_list
        ]
        valid_loaders = [
            DataLoader(valid, batch_size=args.batch_size * 4, shuffle=False, num_workers=args.nw)
            for valid in val_dataset_list
        ]
    else:
    ...
    for task_id, train_loader in enumerate(train_loaders):
        if task_id == 0 and args.firsttask_modelpath != "None":
            ...
            model_path = os.path.join(
                args.firsttask_modelpath,
                f"{args.dataset}_seed_{args.seed}",
                "0_model.pth",
            )
            print(f"Loading model from path {model_path}")
            rollback_model(approach, model_path, device, name=str(task_id))
            epoch = 100
        else:
            print("#" * 40 + f" --> TRAINING HEAD {task_id}")
            approach.pre_train(task_id)
            best_taw_accuracy, best_tag_accuracy = 0, 0
            best_loss = math.inf

            for epoch in range(approach.total_epochs):
                print(f"Epoch {epoch}/{approach.total_epochs}")
                approach.train(task_id, train_loader, epoch, approach.total_epochs)
                taw_acc, tag_acc, test_loss = approach.eval(
                    task_id,
                    task_id,
                    valid_loaders[task_id],
                    epoch,
                    verbose=True,
                )
        ...
        for test_id in range(task_id + 1):
            acc_taw_value, acc_tag_value, _ = approach.eval(
                task_id,
                test_id,
                test_loaders[test_id],
                epoch,
                verbose=False,
            )
            logger.update_accuracy(
                current_training_task_id=task_id,
                test_id=test_id,
                acc_taw_value=acc_taw_value,
                acc_tag_value=acc_tag_value,
            )
            if test_id < task_id:
