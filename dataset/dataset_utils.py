...
def get_train_val_images_imagenet_subset(data_path):
    ...
    train_dir = os.path.join(data_path, 'imagenet100', 'train')
    test_dir = os.path.join(data_path, 'imagenet100', 'val')
    train_dset = datasets.ImageFolder(train_dir)
    test_dset = datasets.ImageFolder(test_dir)
...
def get_train_val_images_cub(data_path: str):
    """
    Parse the official CUB text files and build train/val file lists.
    ...
    Folder layout expected:
        data_path/CUB_200_2011
            ...
            └── (other metadata ...)
    """
    root = os.path.join(data_path, "CUB_200_2011")
    images_dir = os.path.join(root, "images")

    with open(os.path.join(root, "images.txt")) as f:
        id2path = {int(i): p for i, p in (ln.strip().split() for ln in f)}
    with open(os.path.join(root, "train_test_split.txt")) as f:
        id2split = {int(i): int(s) for i, s in (ln.strip().split() for ln in f)}

    # Map class folder name → integer index (sorted for consistency)
    class_names = sorted({p.split("/")[0] for p in id2path.values()})
    ...
    return (train_imgs, np.array(train_lbls),
            val_imgs,   np.array(val_lbls),
            class_to_idx)


def get_train_val_images_stanfordcar(data_path: str):
    """
    Leverage torchvision.datasets.StanfordCars (handles .mat annotations).
    """
    train_raw = torchvision.datasets.StanfordCars(root=data_path,
                                                  split="train",
                                                  download=True)
    test_raw  = torchvision.datasets.StanfordCars(root=data_path,
                                                  split="test",
                                                  download=True)

    train_imgs   = [p for p, _ in train_raw.samples]
    train_lbls   = [l for _, l in train_raw.samples]
    test_imgs    = [p for p, _ in test_raw.samples]
    test_lbls    = [l for _, l in test_raw.samples]
    class_to_idx = train_raw.class_to_idx

    return (train_imgs, np.array(train_lbls),
            test_imgs,  np.array(test_lbls),
            class_to_idx)
